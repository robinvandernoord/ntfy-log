...
def main():
    ...
    for target in targets:
        ...
        os.system(f"rclone copy --progress {binary_path} {RCLONE_ENDPOINT}:{BUCKET_NAME}/{arch}/")

    # update manifest:
    with tempfile.NamedTemporaryFile(suffix="build-release.json") as f:
        os.system(f"rclone copyto --progress {RCLONE_ENDPOINT}:{BUCKET_NAME}/index.json {f.name}")

        f_path = Path(f.name)
        current_contents = json.loads(f_path.read_text()) if (f_path.exists() and not is_empty(f_path)) else {}
        current_contents[BINARY_NAME] = read_cargo()
        f_path.write_text(json.dumps(current_contents))

        os.system(f"rclone copyto --progress {f.name} {RCLONE_ENDPOINT}:{BUCKET_NAME}/index.json")
